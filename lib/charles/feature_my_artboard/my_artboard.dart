...
import 'package:scrapyart_home/charles/feature_my_artboard/stack_image.dart';
...
class _MyArtboardState extends State<MyArtboard> {
  ...
  // void _showContextMenuDynamic(BuildContext context,
  //     {required Layers layer}) async {
  //   final RenderObject? overlay =
  //       Overlay.of(context).context.findRenderObject();
  //   // ignore: unused_local_variable
  //   final result = await showMenu(
    ...
    for (var i = 0; i < globalListImage.length; i++) {
      ...
      data.add(Positioned(
        ...
        child: GestureDetector(
          ...
          onPanUpdate: (details) {
            e.top = max(0, e.top + details.delta.dy);
            e.left = max(0, e.left + details.delta.dx);
            setState(() {});
          },
          child: Image.file(e.image!),
        ),
      ));
    }
